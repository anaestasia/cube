...
export default function Register() {
  ...
  const register = () => {
    ...
    Axios.post("http://localhost:3001/users/create", {
      ...
      country	:nameCountry,
      date_creation:'2021-01-02',//mettre date now en js	
      last_connexion	:'2021-12-22',//mettre date now en js
      checked	:'0',
      deleted	:'0',
      fk_role:'1', //regarder pour faire une recherche sur le nom sinon laisser l'id de la table role
    }).then((response) => {
      console.log(response);
      setregisterStatus('Tu es enregistré');
    });
  };
    ...
    <div className="App">
      <div className="registration">
        ...
        <form onSubmit={register}>
          ...
          <input type="number" onChange={(e) => {setNbStreet(e.target.value); }} required/>
          <br />
          <label>nom rue :</label>
          <input type="text" onChange={(e) => {setnameStreet(e.target.value); }} required/>
          <br />
          <label>nom ville :</label>
          <input type="text" onChange={(e) => {setCityName(e.target.value); }} required/>
          <br />
          <label>code postal ville :</label>
          <input type="text" onChange={(e) => {setpostalCode(e.target.value); }} required/>
          <br />
          <label>code postal ville :</label>
          <input type="text" onChange={(e) => {setnameCountry(e.target.value); }} required/>
          <br />
