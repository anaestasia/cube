...
export default function TableRoles({actionRole}) {
  ...
  const titreModal = 'Role';
  const [nameRole, setNameRole] = useState('AdminLol');
  ...
  return (
    ...
    <Modal show={show} onHide={handleClose}>
      ...
      { actionRole ? <Table titre={"Role"} columns={columnsAction} data={data}/> : <Table titre={"Role"} columns={columns} data={data}/>}
    </>
  );
}