...
export default function TableCategories({actionRole}) {
  
  const nomDeLaTable = "categories"
  //modal 
  const [show, setShow] = useState(false);
  const titreModal = 'Catégories';
  const [nameCategorie, setNameCategorie] = useState('');
  const [ajouterModal, setAjouterModal] = useState(true);

  const handleClose = () => setShow(false);
  //fin modal
  const titreTableau = "Catégories";
  //modal del
  ...
  return (
    ...
    <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          ...
          <input type="text" id="idNameEdit" value={nameCategorie} onChange={(e) => { setNameCategorie(e.target.value); }} />
        </Modal.Body>
        <Modal.Footer>
